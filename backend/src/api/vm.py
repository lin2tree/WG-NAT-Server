...
class ReadyRequest(BaseModel):
    """Request body for /vm/ready endpoint"""
    success: bool = True
...
async def report_vm_ready(
    request: Request,
    body: ReadyRequest = ReadyRequest(),
    source_ip: str = Depends(verify_vm_request),
    db: Session = Depends(get_db),
):
    """Report VM is ready with WireGuard running
    
    VM calls this after setting up WireGuard.
    
    Request body:
        success: True if WireGuard started successfully, False if error occurred
        error_message: Error details if success=False
    
    Updates status from init to started (or error).
    ...
    try:
        error_msg = body.error_message if not body.success else None
        config = vpn_service.report_ready(source_ip, error_message=error_msg)
        ...
        if body.error_message:
            return {
                "success": True,
                "message": f"VM {source_ip} error reported",
                "data": {
                    "vm_ip": config.vm_ip,
                    "status": config.status,
